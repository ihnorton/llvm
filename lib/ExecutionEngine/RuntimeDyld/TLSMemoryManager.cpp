//===-- TLSMemoryManager.cpp - TLS Support for RuntimeDyld --=---*- C++ -*-===//
//
...
TLSMemoryManagerELF::TLSOffset
TLSMemoryManagerGLibC::TLSdlsym(StringRef Name) override {
    ...
    uint64_t Value = MM->getSymbolAddres(Name);
    ...
    return TLSOffset(found_i, found_offset);
}
...
TLSMemoryManagerELF::TLSOffset
TLSMemoryManagerDarwin::TLSdlsym(StringRef Name) {
    // Ask the MemoryManager to dlsym this value for us.
    // The result will be a pointer to the TLVDescriptor.
    uint64_t Value = MM->getSymbolAddress(Name);
    TLVDescriptor *Descriptor = (TLVDescriptor *)Value;
    ...
    return TLSOffset(Descriptor->key, Descriptor->offset);
};
